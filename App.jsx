...
// [요청사항 3] 경영지원본부 회의 의견 작성용 팀 리스트 (별도 입력 필드용)
...
export default function App() {
  ...
  const [minutes, setMinutes] = useState([]); // 일반 부서 회의록 데이터
  const [feedbacks, setFeedbacks] = useState([]); // [요청사항 1] 경영지원본부 회의록 데이터
  const [loading, setLoading] = useState(true);
  
  // [NEW] 뷰 상태 관리 ('minutes': 일반 회의록, 'management': 경영본부)
  const [currentView, setCurrentView] = useState('minutes');

  // 모달 상태 관리
  const [isModalOpen, setIsModalOpen] = useState(false); // 일반 작성 모달
  const [isFeedbackModalOpen, setIsFeedbackModalOpen] = useState(false); // [요청사항 1] 경영지원본부 작성 팝업
  ...
  const allDates = [...new Set(Object.keys(groupedMinutes))].sort((a, b) => b.localeCompare(a));
  ...
  const handleExportCSV = () => {
    ...
    if (currentView === 'minutes') {
        ...
        dataToExport = feedbacks; // 경영본부는 필터 없이 전체 or 필요한 만큼
    ...
    dataToExport.forEach(row => {
        if (currentView === 'minutes') {
            ...
            csvContent += `${row.date},${cleanText(formatTextForExcel(row.finance))},${cleanText(formatTextForExcel(row.hr))},${cleanText(formatTextForExcel(row.global))},${cleanText(formatTextForExcel(row.logistics))},${cleanText(formatTextForExcel(row.it))}\n`;
        }
    });
    ...
    link.setAttribute("download", `주간회의록_${currentView === 'minutes' ? '부서별' : '경영본부'}_${new Date().toISOString().slice(0,10)}.csv`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-800">
      <nav className="bg-white shadow-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center cursor-pointer" onClick={() => setCurrentView('minutes')}>
            ...
            {/* 네비게이션 탭 버튼 */}
            <div className="hidden md:flex space-x-2 items-center mx-4">
               <button
            ...
            <div className="flex items-center space-x-2 sm:space-x-4">
              ...
              {currentView === 'minutes' ? (
                <button
                    ...
                    className="px-3 py-2 rounded-md text-sm font-medium transition-colors bg-blue-600 text-white hover:bg-blue-700 shadow-sm"
                >
                    <div className="flex items-center">
                ...
                <button
                    ...
                    className="px-3 py-2 rounded-md text-sm font-medium transition-colors bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm"
                >
                    <div className="flex items-center">
                    <PlusCircle className="w-4 h-4 mr-2" />
                    <span className="hidden sm:inline">경영본부 의견 작성</span>
                    <span className="sm:hidden">작성</span>
                    </div>
                </button>
              )}
        ...
        <div className="md:hidden flex border-t border-gray-200">
           <button
             ...
             className={`flex-1 py-3 text-sm font-medium text-center ${
               currentView === 'minutes' ? 'bg-blue-50 text-blue-700 border-b-2 border-blue-600' : 'text-gray-500'
             }`}
           >
             부서 주간회의
           </button>
           <button
             onClick={() => setCurrentView('management')}
             className={`flex-1 py-3 text-sm font-medium text-center ${
               currentView === 'management' ? 'bg-indigo-50 text-indigo-700 border-b-2 border-indigo-600' : 'text-gray-500'
             }`}
           >
             경영본부 회의
           </button>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {currentView === 'minutes' && (
            ...
            <div className="space-y-8">
                ...
                filteredDates.map(date => {
                    ...
                    if (filteredMinutes.length === 0) return null;
            ...
            <div className="space-y-6">
                <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-6 mb-6">
                    ...
                    <p className="text-indigo-700">
                        경영지원본부 주관 회의에서 논의된 각 팀별 지시사항 및 의견입니다.
                    </p>
                ...
                {feedbacks.length === 0 ? (
                    <div className="text-center py-12 bg-white rounded-lg border border-dashed border-gray-300">
                        ...
                        <h3 className="mt-2 text-sm font-medium text-gray-900">등록된 경영본부 회의 내용이 없습니다</h3>
                        <p className="mt-1 text-sm text-gray-500">우측 상단의 '작성' 버튼을 눌러 내용을 등록하세요.</p>
                    </div>
                ) : (
                    feedbacks.map((fb) => (
                        <div key={fb.id} className="bg-white rounded-xl shadow border border-indigo-100 overflow-hidden mb-8">
                             <div className="bg-indigo-600 px-6 py-4 flex justify-between items-center">
                                <div className="flex items-center">
                                    <Calendar className="w-5 h-5 text-indigo-200 mr-2" />
                                ...
                                <div className="flex space-x-2">
                                    <button 
                                    onClick={() => handleFeedbackEditClick(fb)}
                                    className="px-3 py-1.5 bg-indigo-500 hover:bg-indigo-400 text-white text-xs rounded transition-colors flex items-center"
                                    >
                                    <Edit className="w-3 h-3 mr-1" /> 수정
                                    </button>
                                    <button 
                                    onClick={() => handleFeedbackDeleteClick(fb.id)}
                                    className="px-3 py-1.5 bg-indigo-500 hover:bg-red-500 text-white text-xs rounded transition-colors flex items-center"
                                    >
                            ...
                            {/* 5열 그리드 레이아웃 - 요청사항 반영 */}
                            <div className="p-6 grid grid-cols-1 md:grid-cols-5 gap-0 md:divide-x md:divide-gray-100">
                                {FEEDBACK_TEAMS.map((team) => (
                                    <div key={team.id} className="px-4 py-2 first:pl-0 last:pr-0">
                                        <h4 className="text-sm font-bold text-indigo-900 mb-3 pb-2 border-b-2 border-indigo-100 flex items-center justify-between">
                                            {team.label}
                                        </h4>
                                        <div className="text-sm text-gray-700 leading-relaxed min-h-[100px]">
                                            {renderFormattedText(fb[team.id])}
                                        </div>
                                    </div>
                                ))}
                            </div>
                            <div className="bg-gray-50 px-6 py-2 border-t border-gray-100 text-xs text-gray-400 text-right">
                                작성일: {new Date(fb.createdAt?.toDate()).toLocaleDateString()}
                            </div>
                        </div>
                    ))
                )}
        ...
        {/* [요청사항 1, 3] 경영지원본부 회의록 작성 모달 (팀별 분리) */}
        {isFeedbackModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
                      ...
                      {FEEDBACK_TEAMS.map((team) => (
                        <div key={team.id} className="bg-white p-3 rounded border border-indigo-100">
                          ...
                          <textarea
                            ...
                            placeholder={`${team.label} 관련 의견을 입력하세요.`}
                            rows={4}
